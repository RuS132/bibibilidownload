        ...
        if response.status_code == 200:
            try:
                ...
                if result.get("success"):
                    ...
                    # tmpfiles.org 返回的是短链，如 https://tmpfiles.org/12345/file.txt
                    # 我们返回可直接下载的链接
                    return file_url
            except requests.exceptions.JSONDecodeError:
                st.error("❌ 服务器返回非 JSON 数据，可能是上传失败或服务异常。")
                st.debug(response.text)  # 可选：调试时打印原始响应
        else:
            st.error(f"❌ 上传失败，HTTP 状态码: {response.status_code}")
    except Exception as e:
        st.error(f"❌ 请求上传服务时出错: {str(e)}")
    return None
